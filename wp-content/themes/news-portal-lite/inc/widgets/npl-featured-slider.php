...
class News_Portal_Lite_Featured_Slider extends WP_widget {
    ...
    public function __construct() {
        $widget_ops = array( 
            'classname' => 'news_portal_lite_featured_slider',
            'description' => __( 'Displays posts from selected categories in slider with featured section.', 'news-portal-lite' )
        );
        parent::__construct( 'news_portal_lite_featured_slider', __( 'NP: Featured Slider', 'news-portal-lite' ), $widget_ops );
    }
    ...
    private function widget_fields() {
        ...
        $fields = array(
            'slider_cat_ids' => array(
                ...
                'news_portal_widgets_title'        => __( 'Slider Categories', 'news-portal-lite' ),
            ...
            'featured_cat_ids' => array(
                ...
                'news_portal_widgets_title'        => __( 'Featured Post Categories', 'news-portal-lite' ),
                'news_portal_widgets_field_type'   => 'multicheckboxes',
                'news_portal_widgets_field_options' => $news_portal_categories_lists
            )
        );
            ...
            <div class="featured-posts">
                ...
                <?php
                    if( !empty( $news_portal_featured_cat_ids ) ) {
                        ...
                        $get_cats_ids = implode( ",", $checked_cats );
                        $news_portal_post_count = apply_filters( 'news_portal_slider_featured_posts_count', 3 );
                        $np_featured_args = array(
                                'post_type' => 'post',
                                'cat'       => esc_attr( $get_cats_ids ),
                                'posts_per_page' => absint( $news_portal_post_count )
                            );
                        ...
                        if( $np_featured_query->have_posts() ) {
                            ...
                            while( $np_featured_query->have_posts() ) {
                                ...
                                if( $post_count == 1 ) {
                                    echo '<div class="featured-top-section np-clearfix">';
                                    $featured_image_size = 'news-portal-lite-horizontal-thumb';
                                } elseif( $post_count == 2 ) {
                                    echo '<div class="featured-bottom-section np-clearfix">';
